...
Route::get('/worlds', function () {
    $worlds = \App\Models\World::active()->ordered()->get();

    return response()->json(['data' => $worlds]);
});

Route::get('/worlds/{slug}', function ($slug) {
    $world = \App\Models\World::where('slug', $slug)->firstOrFail();
...
Route::get('/worlds/{slug}/categories', function ($slug) {
    $world = \App\Models\World::where('slug', $slug)->firstOrFail();
    $categories = $world->categories()
        ->active()
        ->root()
...
Route::post('/businesses/{slug}/bookings', [PublicBookingController::class, 'storeBooking'])->middleware('auth.optional');
Route::post('/businesses/{slug}/orders', [PublicBookingController::class, 'storeOrder'])->middleware('auth.optional');
...
Route::post('/businesses/{slug}/trips', [TransportController::class, 'bookTrip'])->middleware('auth.optional');

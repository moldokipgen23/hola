...
@section('title', 'Turf Management')
@section('header', 'Turf / Sports Venue Management')
...
<div class="max-w-5xl mx-auto space-y-5">
    <div class="glass-card p-6 rounded-lg">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-slate-400 mt-1">Manage courts, pitches, slots, and availability for turf bookings.</p>
            </div>
        ...
        <div class="flex justify-between items-start mb-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">
                    Duration: {{ $service->duration }} min · Price: ₹{{ $service->price }} per slot · Capacity: {{ $service->capacity }} units
                </p>
            </div>
            <form method="POST" action="{{ route('vendor.businesses.turf.toggle', [$business->id, $service->id]) }}">
                @csrf @method('PUT')
    ...
    <div class="glass-card p-10 rounded-lg text-center">
        <p class="text-slate-400 mb-3">No turf services found.</p>
        <a href="{{ route('vendor.services.create', $business->id) }}" class="btn-primary">Create a Turf Service</a>
    </div>
    @endforelse
</div>
@endsection

    ...
    <form method="POST" action="{{ isset($category) ? route('admin.categories.update', $category->id) : route('admin.categories.store') }}">
        ...
        <div class="glass-card p-6 rounded-lg space-y-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_canonical" value="1" {{ old('is_canonical', $category->is_canonical ?? 1) ? 'checked' : '' }}>
                    <span class="text-sm text-slate-300">Approved taxonomy</span>
                </label>
            </div>

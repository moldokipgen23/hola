        ...
        <aside class="sidebar w-64 flex flex-col mobile-menu" id="sidebar">
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                <div>
                    ...
                    <p class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider px-4 mb-2 mt-6">Directory + AI Scout</p>
                    ...
                    <a href="{{ route('admin.claims') }}" class="sidebar-link {{ request()->routeIs('admin.claims*') ? 'active' : '' }}">
                        ...
                        <span>Claims</span>
                        @if(($adminBadges['pending_claims'] ?? 0) > 0)<span class="sidebar-badge">{{ $adminBadges['pending_claims'] }}</span>@endif
                    </a>
                    <a href="{{ route('admin.import') }}" class="sidebar-link {{ request()->routeIs('admin.import*') ? 'active' : '' }}">
                        ...
                        @if(($adminBadges['pending_imports'] ?? 0) > 0)<span class="sidebar-badge">{{ $adminBadges['pending_imports'] }}</span>@endif
                    </a>

                    <p class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider px-4 mb-2 mt-6">Business Types</p>
                    <a href="{{ route('admin.businesses-type.shopping') }}" class="sidebar-link {{ request()->routeIs('admin.businesses-type.shopping') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('admin.businesses-type.booking') }}" class="sidebar-link {{ request()->routeIs('admin.businesses-type.booking') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/></svg>
                        <span>Booking</span>
                    </a>
                    <a href="{{ route('admin.businesses-type.taxi') }}" class="sidebar-link {{ request()->routeIs('admin.businesses-type.taxi') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.146-.504 1.2-1.125a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25m-2.25 0h-2.25m0 0v-1.5c0-.33.176-.625.44-.781a2.25 2.25 0 012.25.781v.781m-4.5 0v3.75m0 0v3.75m0-3.75h.008v-.008h-.008v.008zm0 3.75h.008v-.008h-.008v.008zm0 3.75h.008v-.008h-.008v.008z"/></svg>
                        <span>Taxi / Transport</span>
                    </a>

                    <p class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider px-4 mb-2 mt-6">Sellers</p>
                    <a href="{{ route('admin.vendors') }}" class="sidebar-link {{ request()->routeIs('admin.vendors*') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349m-16.5 11.65V9.35m0 0a3.001 3.001 0 003.75-.615A2.993 2.993 0 009.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 002.25 1.016c.896 0 1.7-.393 2.25-1.016a3.001 3.001 0 003.75.614m-16.5 0a3.004 3.004 0 01-.621-4.72L4.318 3.44A1.5 1.5 0 015.378 3h13.243a1.5 1.5 0 011.06.44l1.19 1.189a3 3 0 01-.621 4.72m-13.5 8.65h3.75a.75.75 0 00.75-.75V13.5a.75.75 0 00-.75-.75H6.75a.75.75 0 00-.75.75v3.75c0 .414.336.75.75.75z"/></svg>
                        <span>All Sellers</span>
                    </a>

                    <p class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider px-4 mb-2 mt-6">Commerce</p>
                    <a href="{{ route('admin.bookings') }}" class="sidebar-link {{ request()->routeIs('admin.bookings*') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/></svg>
                        <span>Bookings</span>
                        @if(($adminBadges['pending_bookings'] ?? 0) > 0)<span class="sidebar-badge">{{ $adminBadges['pending_bookings'] }}</span>@endif
                    </a>

                    <p class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider px-4 mb-2 mt-6">Fulfillment</p>
                    ...
                    <p class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider px-4 mb-2 mt-6">Customers</p>
                    ...
                    <a href="{{ route('admin.reports') }}" class="sidebar-link {{ request()->routeIs('admin.reports*') ? 'active' : '' }}">
                        ...
                        <span>Reports</span>
                        @if(($adminBadges['pending_reports'] ?? 0) > 0)<span class="sidebar-badge">{{ $adminBadges['pending_reports'] }}</span>@endif
                    </a>

                    <p class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider px-4 mb-2 mt-6">Content</p>
                    ...
                    <a href="{{ route('admin.area-interests') }}" class="sidebar-link {{ request()->routeIs('admin.area-interests*') ? 'active' : '' }}">
                        ...
                        <span>Coming-soon Interest</span>
                    </a>
                    ...
                    <a href="{{ route('admin.feature-flags') }}" class="sidebar-link {{ request()->routeIs('admin.feature-flags*') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5.636 5.636a9 9 0 1012.728 0M12 3v9"/></svg>
                        <span>Launch Controls</span>
                        @if(($adminBadges['enabled_flags'] ?? 0) > 0)<span class="sidebar-badge">{{ $adminBadges['enabled_flags'] }} ON</span>@endif
                    </a>
                    <a href="{{ route('admin.settings') }}" class="sidebar-link {{ request()->routeIs('admin.settings') ? 'active' : '' }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.28z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        <span>Settings</span>
                    </a>
                    <a href="{{ route('admin.capability-templates') }}" class="sidebar-link {{ request()->routeIs('admin.capability-templates*') ? 'active' : '' }}">

...
class ImportPincodes extends Command
{
    protected $signature = 'pincodes:import
        ...
        {--enable-state=* : State names that should be transaction-serviceable}
        {--enable-district=* : District names that should be transaction-serviceable}';
    ...
    {
        if (! $this->option('truncate') && Pincode::query()->exists()) {
            $this->error('Pincodes already exist. Re-run with --truncate to replace the master dataset safely.');
        ...
        if (! $file) {
            ...
            $contents = @file_get_contents($url, false, stream_context_create([
                'http' => ['timeout' => 60, 'user_agent' => 'EihoOneApp/1.0'],
            ]));
            if ($contents === false) {
        ...
        $count = 0;
        $batchSize = 500;
        $enabledStates = collect($this->option('enable-state'))->map(fn ($value) => strtolower(trim($value)));
        $enabledDistricts = collect($this->option('enable-district'))->map(fn ($value) => strtolower(trim($value)));
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            $serviceable = $enabledStates->contains(strtolower($state))
                || $enabledDistricts->contains(strtolower($district));
